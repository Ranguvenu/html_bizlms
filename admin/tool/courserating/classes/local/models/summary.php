...
class summary extends \core\persistent {
     ...
     * @return summary
     */
    public static function get_for_course(int $componentid,string $componentname=''): summary {
        global $DB;
       if(!empty($componentname)){
             if ($summary = self::get_records_select("componentid = :componentid AND component LIKE :component", ['componentid' => $componentid,'component' => $componentname])) {
                ...
                return array_values($summary)[0];
            } else {
                $summary = new static(0, (object)[
                    'componentid' => $componentid,
                    'ratingmode' => helper::get_course_rating_mode($componentid),
             ...
             return $summary = new static(0, (object)[]);
        }
     ...
     * @return static
     */
    public static function add_rating(int $componentid, rating $rating): self {
        
        if (!$record = self::get_record(['componentid' => $componentid,'component'=>$rating->get('component')])) {
            $record = new self(0, (object)['componentid' => $componentid]);
        }
        $record->set('cntall', $record->get('cntall') + 1);
        //$record->set('component', $rating->get('component'));
     ...
     * @return static|null
     */
    public static function update_rating(int $componentid, rating $rating, \stdClass $oldrecord): ?self {

        global $CFG ,$PAGE;
        $ratingold = $oldrecord->rating;
        $summary = self::get_for_course($componentid,$rating->get('component'));
        ...
        //$summary->set('component', $rating->componentname);
        if ($rating != $ratingold) {
            //$summary->set('component', $summary->get('component'));
    ...
    public function recalculate(): ?self {
        ...
        $sql = 'SELECT COUNT(id) AS cntall,
            ...
            WHERE r.componentid = :componentid AND r.component = :component
        ';
        $params = ['componentid' => $this->get('componentid'),'component'=>$this->get('component')];
    ...
    public static function delete_rating(\stdClass $rating): ?self {
        ...
        $record = self::get_for_course($rating->componentid,$rating->component);
