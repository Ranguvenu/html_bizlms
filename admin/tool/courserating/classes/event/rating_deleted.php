...
class rating_deleted extends \core\event\base {
    ...
    public static function create_from_rating(\stdClass $recordold, int $flagcount, string $reason): self {
        /** @var self $event */
        $context = ($recordold->component == 'local_courses' ) ? \context_course::instance($recordold->componentid) : \context_system::instance();
        $event = static::create([
            'objectid' => $recordold->id,
            'componentid' => $recordold->componentid,
            'component' => $recordold->component,
            'context' => $context,
           // 'context' => \context_course::instance($recordold->componentid, IGNORE_MISSING) ?? \context_system::instance(),
