...
class rating_exporter extends persistent_exporter {
    ...
    protected function get_format_parameters_for_review() {
        return [
            ...
            'filearea' => 'review',
            'itemid' => $this->data->id,
            'context' => \context_course::instance($this->data->componentid),
        ];
    }
    ...
    protected function get_other_values(\renderer_base $output): array {
        ...
        if (permission::can_delete_rating($this->data->id, $this->data->componentid)) {
            $flags = flag::count_records(['ratingid' => $this->data->id]);
        } else {
            $flags = [];
        }
        $flagged = flag::get_records(['ratingid' => $this->data->id, 'userid' => $USER->id]) ? true : false;
        //$ratingid = rating::get_record(['id' => $this->data->id, 'userid' => $USER->id]);
        $ratingid = $DB->get_field('tool_courserating_rating','id', array('id'=>$this->data->id,'userid' =>$USER->id));
        if(is_siteadmin() || ($this->data->id == $ratingid)){
            $checkflagged =$flagged;
