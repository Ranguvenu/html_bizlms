...
class helper {
     ...
     */
    protected static function get_custom_field_data(int $componentid, string $shortname): ?data_controller {
        if ($f = self::find_custom_field_by_shortname($shortname)) {
            $fields = \core_customfield\api::get_instance_fields_data([$f->get('id') => $f], $componentid);
            foreach ($fields as $data) {
                if (!$data->get('id')) {
                    $data->set('contextid', \context_course::instance($componentid)->id);
                }
                return $data;
            }
        }
     ...
     * @return data_controller|null
     */
    public static function get_course_rating_data_in_cfield(int $componentid): ?data_controller {
        return self::get_custom_field_data($componentid, constants::CFIELD_RATING);
    }
     ...
     * @return data_controller|null
     */
    public static function get_course_rating_enabled_data_in_cfield(int $componentid): ?data_controller {
        return self::get_custom_field_data($componentid, constants::CFIELD_RATINGMODE);
    }
     ...
     * @return int
     */
    public static function get_course_rating_mode(int $componentid): int {
        $mode = self::get_setting(constants::SETTING_RATINGMODE);
        if (self::get_setting(constants::SETTING_PERCOURSE)) {
            if ($data = self::get_course_rating_enabled_data_in_cfield($componentid)) {
    ...
    public static function format_review($value, \stdClass $row): string {
        ...
        // if(){}
        $context = (!empty($row->componentid) && $row->component == 'local_courses') ? \context_course::instance($row->componentid) : \context_system::instance();
    ...
    public static function format_actions($id, $row): string {
        if (!$id || !permission::can_delete_rating($id, $row->componentid)) {
            return '';
        }
