...
class api {
     ...
     * @return rating
     */
    public static function set_rating(int $componentid, \stdClass $data, int $userid = 0): rating {
        global $USER;
        ...
        $ratingold = rating::get_record(['userid' => $userid, 'componentid' => $componentid,'component' => $componentname]);
        if ($ratingold) {
            ...
            $r->set('rating', $rating);
            $r->set('review', $review);
            $r->set('component', $componentname);
            $r->save();
            $summary = summary::update_rating($componentid, $r, $oldrecord);
        } else {
            ...
            $summary = summary::add_rating($componentid, $r);
        }
    ...
    public static function delete_rating(int $ratingid, string $reason = ''): ?rating {
        ...
        $rating->delete();
        if ($context = \context_course::instance($record->componentid, IGNORE_MISSING)) {
    ...
    protected static function update_course_rating_in_custom_field(?summary $summary) {
        ...
        if ($summary->get('ratingmode') == constants::RATEBY_NOONE) {
            ...
            $data = (new summary_exporter($componentid,$component, $summary))->export($output);
            $ratingstr = $output->render_from_template('tool_courserating/summary_for_cfield', $data);
        }
        if($component == "local_courses"){
             if ($data = helper::get_course_rating_data_in_cfield($componentid)) {
    ...
    protected static function prepare_review(?rating $rating, \stdClass $data): string {
        ...
        if ($rating && $usehtml && !rating::review_is_empty($data->review_editor['text'] ?? '')) {
            $context = \context_course::instance($rating->get('componentid'));
     ...
     * @return array|array[]
     */
    public static function prepare_rating_for_form(int $componentid,string $componentname): array {
        global $USER;
        ...
        if ($rating = rating::get_record(['userid' => $USER->id, 'componentid' => $componentid,'component' => $componentname])) {
     ...
     * @return void
     */
    public static function reindex(int $componentid = 0) {
        global $DB, $SITE,$PAGE;
        ...
        $fields = 'c.id as componentid, d.value as cfield, s.cntall as summarycntall, s.ratingmode as summaryratingmode,
               (select count(1) from {tool_courserating_rating} r where r.componentid=c.id) as actualcntall ';
        $join = 'from {course} c
            left join {tool_courserating_summary} s on s.componentid = c.id
        ...
        foreach ($records as $record) {
            ...
            $record->component = "local_courses";
     ...
     *
     * @param \stdClass $data contains fields: componentid, cfield, summarycntall, actualcntall
    ...
    protected static function reindex_course(\stdClass $data) {
        ...
        if ($data->summaryratingmode != $data->actualratingmode) {
            ...
            $summary = summary::get_for_course($data->componentid,$data->component);
        ...
        if ($mustbeempty) {
            ...
            if (!empty($data->cfield)) {
                $summary = $summary ?? summary::get_for_course($data->componentid,$data->component);
                self::update_course_rating_in_custom_field($summary);
            }
        } else {
     ...
     * @return void
     */
    public static function delete_all_data_for_course(int $componentid) {
        global $DB;
        $DB->execute('DELETE from {'.flag::TABLE.'} WHERE ratingid IN (SELECT id FROM {'.
            rating::TABLE.'} WHERE componentid = ?)', [$componentid]);
        $DB->delete_records(rating::TABLE, ['componentid' => $componentid]);
        $DB->delete_records(summary::TABLE, ['componentid' => $componentid]);
    }
}
