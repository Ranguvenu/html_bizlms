...
final class api_test extends \advanced_testcase {
     ...
     */
    protected function assert_rating(?array $expected, int $userid, int $componentid) {
        global $DB;

        $params = ['componentid' => $componentid, 'userid' => $userid];
        if (empty($expected)) {
     ...
     * @param int $componentid
     */
    protected function assert_summary(array $expected, int $componentid) {
        global $DB;
        ...
        $cfield = $DB->get_field_sql('SELECT d.value FROM {customfield_field} f
            ...
            [constants::CFIELD_RATING, $componentid, 'core_course', 'course']);
     ...
     * @return void
     */
    protected function assert_event(\phpunit_event_sink $sink, string $classname, int $componentid,
        ...
        $this->assertEquals($componentid, $event->componentid);
        $this->assertEquals(\context_course::instance($componentid)->id, $event->contextid);
    ...
    public function test_delete_rating(): void {
        ...
        $expected = ['cntall' => 2, 'avgrating' => 3, 'sumrating' => 6, 'cnt02' => 1, 'cnt03' => 0, 'cnt04' => 1];
        $this->assert_summary($expected, $course->id);
        $rating = rating::get_record(['userid' => $user->id, 'componentid' => $course->id]);
