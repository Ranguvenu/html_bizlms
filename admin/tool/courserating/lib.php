...
function tool_courserating_before_footer() {
    ...
    if (\tool_courserating\helper::course_ratings_enabled_anywhere()) {
        ...
        if (($componentid = \tool_courserating\helper::is_course_page()) ||
            ...
            $res .= $output->course_rating_block($componentid,"local_courses");
        }
    }
    return $res;
}
...
function tool_courserating_output_fragment_course_ratings_popup($args) {
    global $PAGE;
    if (!$componentid = clean_param($args['componentid'] ?? 0, PARAM_INT)) {
        throw new moodle_exception('missingparam', '', '', 'componentid');
    }
     if (!$componentname = clean_param($args['componentname'] ?? 0, PARAM_RAW)) {
        throw new moodle_exception('missingparam', '', '', 'componentname');
    }
    \tool_courserating\permission::require_can_view_ratings($componentid,$componentname);
    /** @var tool_courserating\output\renderer $output */
    $output = $PAGE->get_renderer('tool_courserating');
    return $output->course_ratings_popup($componentid,$componentname);
}
...
function tool_courserating_output_fragment_cfield($args) {
    ...
    if (!$componentid = clean_param($args['componentid'] ?? 0, PARAM_INT)) {
     ...
     if (!$component = clean_param($args['componentname'] ?? "", PARAM_RAW)) {
        throw new moodle_exception('missingparam', '', '', 'componentname');
    }
    \tool_courserating\permission::require_can_view_ratings($componentid,$component);
    /** @var tool_courserating\output\renderer $output */
    $output = $PAGE->get_renderer('tool_courserating');
    return $output->cfield($componentid,$component);
}
...
function tool_courserating_output_fragment_course_ratings_summary($args) {
    ...
    /** @var tool_courserating\output\renderer $output */
    $output = $PAGE->get_renderer('tool_courserating');
    $data = (new \tool_courserating\external\summary_exporter(0,$componentname))->export($output);
...
function tool_courserating_output_fragment_rating_flag($args) {
    ...
    \tool_courserating\permission::require_can_view_ratings($rating->get('componentid'),$rating->get('component'));
...
function tool_courserating_output_fragment_course_reviews($args) {
    global $PAGE;
    $args = [
        'componentid' => clean_param($args['componentid'] ?? 0, PARAM_INT),
        'component' => clean_param($args['component'] ?? "", PARAM_RAW),
    ...
    if (!$args['component']) {
        throw new moodle_exception('missingparam', '', '', 'component');
    }
    \tool_courserating\permission::require_can_view_ratings($args['componentid'],$args['component']);
    /** @var tool_courserating\output\renderer $output */
    $output = $PAGE->get_renderer('tool_courserating');
...
function tool_courserating_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    \tool_courserating\permission::require_can_view_ratings($context->instanceid,null);
    ...
    $rating = \tool_courserating\local\models\rating::get_record(['componentid' => $context->instanceid, 'id' => $itemid]);
    if (!$rating) {
        return false;
    }
