...
function notification_details($tablelimits, $filtervalues) {
    ...
    $selectsql = "SELECT ni.id, nt.name, nt.shortname, ni.subject, ni.open_path,ni.moduletype,ni.moduleid, lc.fullname as deptname, ni.active
                FROM {local_notification_info} ni
    ...
    if ($notifications_info) {
        foreach ($notifications_info as $each_notification) {
            ...
            if ($each_notification->moduleid) {
                $moduleids = explode(",", $each_notification->moduleid);

                foreach ($moduleids as $module) {
                    if ($each_notification->moduletype == 'classroom') {
                        $course = $DB->get_field('local_classroom', 'name', array('id' => $module));
                    } else if ($each_notification->moduletype == 'feedback') {
                        $course = $DB->get_field('local_evaluations', 'name', array('id' => $module));
                    } else if ($each_notification->moduletype == 'learningplan') {
                        $course = $DB->get_field('local_learningplan', 'name', array('id' => $module));
                    } else if ($each_notification->moduletype == 'onlineexam') {
                        $course = $DB->get_field('course', 'fullname', array('id' => $module, 'open_coursetype' => 'online_exams'));
                    } else if ($each_notification->moduletype == 'program') {
                        $course = $DB->get_field('local_program', 'name', array('id' => $module));
                    } else if ($each_notification->moduletype == 'request') {
                        $course = $DB->get_field('course_request', 'fullname', array('id' => $module));
                    } else if ($each_notification->moduletype == 'course') {
                        $course = $DB->get_field('course', 'fullname', array('id' => $module));
                    }
                    array_push($coursenames, $course);
                }
            }
            $list['courses'] = !empty($coursenames) ? implode(',', $coursenames) : '-';
            $data[] = $list;
        }
    }
