...
function get_module_custom_categories($moduleid, $moduletype) {
    ...
    $custom_categories_sql = "SELECT cc.id,cc.parentid,cc.costcenterid FROM {local_custom_category} cc
                                JOIN {course} c ON c.performancecatid = cc.id
                                WHERE c.id = :moduleid ";
...
function user_performance_logs($event, $moduletype, $moduleid, $performance) {
    ...
    if ($performance_obj->type == 0) {
        ...
        $total_internalpoints = $DB->get_field_sql(
            $sql . $wheresql . $groupbysql,
...
function custom_matrix_data($data) {
    ...
    if (count($result) == 0) {
        ...
        if ($typecount > 0) {
            ...
            $internal_type = $DB->get_records('local_custom_category', array('costcenterid' => $data['costcenter'], 'type' => 0, 'parentid' => 0));
...
function local_custom_matrix_leftmenunode() {
    ...
    if ($matrixpluginstatus && (has_capability('local/custom_matrix:manage_custom_matrix', $categorycontext) || is_siteadmin())) {
