...
class querylib {
    ...
    function join_performance_matrix_records($data) {
        $sql = "SELECT cm.id, cm.fullname, cm.parentid,cm.type,pm.id as pmid,pm.maxscore,pm.weightage,pm.role,pm.type
                    FROM {local_custom_category} cm
                    LEFT JOIN {local_performance_matrix} pm ON cm.id=pm.performancecatid
                    WHERE cm.costcenterid = :costcenter AND role = :role";
    ...
    function join_performance_matrix_record($data) {
        $sql = "SELECT cm.id, cm.fullname, cm.parentid,cm.type,pm.id as pmid,pm.maxscore,pm.weightage,pm.role,pm.type
                    FROM {local_custom_category} cm
                    LEFT JOIN {local_performance_matrix} pm ON cm.id=pm.performancecatid
                    WHERE costcenterid = :costcenter AND role = :role";
        if (isset($data['performancecatid'])) {
            $sql .= " AND pm.performancecatid = :performancecatid";
        }
        if (isset($data['type'])) {
