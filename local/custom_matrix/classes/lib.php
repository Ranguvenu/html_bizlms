...
class lib {
    ...
    public function custom_matrix_operations($formdata) {
        ...
        if ($formdata->id) {
            ...
            $data->path = $path;
            $matrixid->id = $this->db->update_record(self::TABLE_CUSTOM_MATRIX, $data, $returnid = true);
        } else {
            $data->timecreated  = time();
            $data->usercreated  = $this->user->id;

            $matrixid->id = $this->db->insert_record(self::TABLE_CUSTOM_MATRIX, $data, $returnid = true);

            if ($matrixid->id) {
                $parentpath = $this->querylib->get_matrixfield('path', array('id' => $formdata->parentid));
                $path = $parentpath . '/' . $matrixid->id;
                $datarecord = new \stdClass();
                $datarecord->id = $matrixid->id;
