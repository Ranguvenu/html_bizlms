...
function xmldb_local_learningplan_upgrade($oldversion) {
    ...
    if ($oldversion < 2022101800.20) {
        $table = new xmldb_table('local_learningplan');
        $field = new xmldb_field('open_project', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2022101800.20, 'local', 'learningplan');
    }
