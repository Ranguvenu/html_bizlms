        ...
        if ($data) {
            ...
            if ($parentid) {
                ...
                foreach ($parentid as $categoryid) {
                    ...
                    $childcategories = $DB->get_field('local_custom_fields_mapped', 'category', array('parentid' => $categoryid->id, 'moduletype' => 'learningplan', 'moduleid' => $data->id));
...
function local_learningplan_leftmenunode() {
    ...
    return array('6' => $learningplannode);
}
...
function get_listof_learningplans($stable, $filterdata, $options = array()) {
    ...
    if (!empty($parentcatids)) {
        $categoryquery = get_moduleid_of_mapped_customfield($filterdata, $parentcatids, 'learningplan', 'l.id');
    ...
    if ($category_plugin_exists) {
        if (!empty($filterdata->categories)) {
            ...
            list($filtercategoriessql, $filtercategoriesparams) = $DB->get_in_or_equal($filtercategories, SQL_PARAMS_NAMED, 'learningplan', true, false);
            $params = array_merge($params, $filtercategoriesparams);
            $fromsql .= " AND (l.open_categoryid $filtercategoriessql) ";
        }
    }
