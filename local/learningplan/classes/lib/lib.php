...
class lib
{
	...
	function update_learning_plan($data)
	{
		...
		if ($data->form_status == 1) {
			...
			$data->open_project = (!empty($data->open_project)) ? implode(',', array_filter($data->open_project)) : NULL;
			if (!empty($data->open_project)) {
				$data->open_project = $data->open_project;
			} else {
				$data->open_project = NULL;
			}
		}
