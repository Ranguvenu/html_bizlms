    ...
    public function get_learningpathlist_query($perpage, $startlimit, $return_noofrecords = false, $returnobjectlist = false, $filters = array()) {
        ...
        if (!is_siteadmin()) {
            ...
            if (!empty($USER->open_hrmsrole) && $USER->open_hrmsrole != "" && $USER->open_hrmsrole != NULL) {
                $wheresql .= " AND ( concat(',',llp.open_hrmsrole,',') LIKE '%,$USER->open_hrmsrole,%'  OR llp.open_hrmsrole = '-1' OR llp.open_hrmsrole = '' OR llp.open_hrmsrole IS NULL OR llp.open_hrmsrole = '_qf__force_multiselect_submission')";
            }
            if (!empty($USER->open_project) && $USER->open_project != "" && $USER->open_project != NULL) {
                $wheresql .= " AND ( concat(',',llp.open_project,',') LIKE '%,$USER->open_project,%'  OR llp.open_project = '-1' OR llp.open_project = '' OR llp.open_project IS NULL OR llp.open_project = '_qf__force_multiselect_submission')";
            }
        }
        $sqlparams = [];
        $i = 0;
        foreach ($filters as $filtertype => $filtervalues) {
            switch ($filtertype) {
                ...
                case 'status':
                    ...
                    //         $wheresql .= " AND llp.id IN (SELECT lcm.moduleid FROM {local_custom_fields_mapped} AS lcm WHERE lcm.moduletype LIKE 'learningplan' AND ($catdependencysql) )";
                    //     }
                    // break;
                default:
