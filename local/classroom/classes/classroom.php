...
class classroom {
    ...
    public function manage_classroom($classroom) {
        ...
        try {
            if ($classroom->id > 0) {
                ...
                $classroom->open_group = ($classroom->open_group == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$classroom->open_group);
                $classroom->open_designation = ($classroom->open_designation == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$classroom->open_designation);
                ...
                if ($classroom->open_costcenterid != $org) {
                    ...
                    customfield_mapping($classroomcat);
                }
    ...
    public function target_audience($classroom) {
        ...
        if ($classroom->id > 0) {
            ...
            $classroom->open_project    = (!empty($classroom->open_project)) ? implode(',', array_filter($classroom->open_project)) : null;
            if (!empty($classroom->open_project)) {
                $classroom->open_project = $classroom->open_project;
            } else {
                $classroom->open_project = NULL;
            }
            $classroom->open_designation = ($classroom->open_designation == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$classroom->open_designation);
    ...
    public function classroom_logo($classroomlogo = 0) {
        ...
        if (!empty($classroomlogorecord)) {
            if ($classroomlogorecord->filearea == "classroomlogo") {
                $classroomlogourl = moodle_url::make_pluginfile_url(
                    $classroomlogorecord->contextid,
                    $classroomlogorecord->component,
                    $classroomlogorecord->filearea,
                    $classroomlogorecord->itemid,
                    $classroomlogorecord->filepath,
    ...
    public  function get_capabilitycheck_list() {
        ...
        return $list = array(
            'viewrecord_capability' => $viewrecord_capability,
        ...
        if (!empty($classroomusers)) {
            foreach ($classroomusers as $classroomuser) {
                ...
                if (empty($classroomcompletiondata) || ($classroomcompletiondata->sessiontracking == null && $classroomcompletiondata->coursetracking == null)) {
                    ...
                    if (
                        !empty($classroomcompletiondata) && $classroomcompletiondata->sessiontracking == "OR" &&
                    ...
                    if (
                        count($usercousrecompletionstatus) <= count($classroomcourses) && count($usercousrecompletionstatus) != 0
