...
class classroom_form extends moodleform {
    ...
    public function definition() {
        ...
        if ($formstatus == 0) {
            ...
            if ($plugin_exists) {
                ...
                $ptype_categories = $DB->get_records_sql($sql, array('parentid' => 0, 'type' => 0, 'costcenterid' => $costcenterid));
                if ($ptype_categories) {
                ...
                $mform->addElement('select', 'open_categoryid', get_string('category'), $performance_types);
                $mform->addRule('open_categoryid', null, 'required', null, 'client');
            }
            ...
            $allowmultisession[] = $mform->createElement(
                'radio',
                'allow_multi_session',
                '',
                get_string('fixed', 'local_classroom'),
                1,
                $attributes
            );
            $allowmultisession[] = $mform->createElement(
                ...
                'allow_multi_session',
                '',
                get_string('custom', 'local_classroom'),
                0,
                $attributes
            );
            $mform->addGroup(
                $allowmultisession,
                'radioar',
                get_string('allow_multi_session', 'local_classroom'),
                array('&nbsp;&nbsp;'),
                false
            );
            ...
            $mform->addElement(
                'text',
                'capacity',
                get_string('capacity', 'local_classroom'),
                array()
            );
            ...
            if (is_siteadmin() || ((has_capability('local/classroom:viewwaitinglist_userstab', $categorycontext)))) {
                ...
                $allowwaitinglistusers[] = $mform->createElement(
                    'radio',
                    'allow_waitinglistusers',
                    '',
                    get_string('yes'),
                    1,
                    $attributes
                ...
                $mform->addGroup(
                    $allowwaitinglistusers,
                    'allow_waitinglistusers',
                    get_string('allow_waitinglistusers', 'local_classroom'),
                    array('&nbsp;&nbsp;'),
                    false
                );
            ...
            if (!empty($trainerslist)) {
                ...
                $trainerslist = $DB->get_records_menu(
                    'local_classroom_trainers',
            ...
            if (!empty($trainerslist)) {
                $trainers = $querieslib->get_user_department_trainerslist(
                    false,
                    false,
                    $trainerslist
                );
            ...
            $mform->addElement('date_time_selector', 'startdate', get_string(
                'startdate',
                'local_classroom'
            ), array('optional' => false));
            $mform->addRule('startdate', null, 'required', null, 'client');

            $mform->addElement('date_time_selector', 'enddate', get_string(
                'enddate',
                'local_classroom'
            ), array('optional' => false));
            ...
            get_custom_fields($costcenterid, $mform, $moduletype = 'classroom', $parentid);
            ...
            $allowmultisession[] = $mform->createElement(
                ...
                'institute_type',
                '',
                get_string('internal', 'local_classroom'),
                1,
                $attributes
            );
            $allowmultisession[] = $mform->createElement(
                ...
                'institute_type',
                '',
                get_string('external', 'local_classroom'),
                2,
                $attributes
            );
            $mform->addGroup($allowmultisession, 'radioar', get_string(
                'clrm_location_type',
                'local_classroom'
            ), array(' '), false);
            $mform->addHelpButton('radioar', 'classroom_locationtype', 'local_classroom');

            $classroomlocations =  array(0 => get_string(
                'select_institutions',
                'local_classroom'
            ));
            ...
            if (!empty($instituteid)) {
                $classroomlocations = $DB->get_records_menu(
                    'local_location_institutes',
                    array('id' => $instituteid),
                    'id',
                    'id, fullname'
                );
            }
            ...
            $mform->addElement('autocomplete', 'instituteid', get_string(
                'classroom_location',
                'local_classroom'
            ), $classroomlocations, $options);
            ...
            $mform->addElement(
                'filepicker',
                'classroomlogo',
                get_string('classroomlogo', 'local_classroom'),
                null,
            ...
            $mform->addElement('editor', 'cr_description', get_string(
                'description',
                'local_classroom'
            ), null, $editoroptions);
            ...
            if (function_exists($functionname)) {
                ...
                $functionname($mform, array('group', 'designation', 'hrmsrole', 'project'), $org);
            }
    ...
    public function validation($data, $files) {
        ...
        if (isset($data['open_categoryid'])) {
            if ($data['open_categoryid'] == 0) {
                $errors['open_categoryid'] = get_string('pleaseselectcategory', 'local_courses');
            }
        }
    ...
    public function set_data($components) {
        ...
        if ($components->form_status == 0) {
            ...
            // $data->open_project = (!empty($data->open_project)) ? array_diff(explode(',', $data->open_project), array('')) : array(NULL => NULL);
            ...
            if ($parentid) {
                ...
                foreach ($parentid as $categoryid) {
                    ...
                    $childcategories = $DB->get_field('local_custom_fields_mapped', 'category', array('parentid' => $categoryid->id, 'moduletype' => 'classroom', 'moduleid' => $data->id));
                    $data->{'customfield_' . $categoryid->id} = $childcategories ? $childcategories : 0;
                }
                $data->parentid = implode(',', $parentcat);
            }
        } else if ($components->form_status == 2) {
            $data = $DB->get_record_sql('SELECT id, description, approvalreqd,
             classroomlogo, nomination_startdate, nomination_enddate,open_skill,open_level
             FROM {local_classroom} WHERE id = ' . $components->id);
            $data->cr_description['text'] = $data->description;
            $data->open_skill = $data->open_skill;
            $data->open_level = $data->open_level;
