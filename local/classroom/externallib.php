...
class local_classroom_external extends external_api {
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewsessions(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewcourses(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewusers(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewfeedbacks(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewrequestedusers(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
    ...
    public static function classroom_instance($id, $categorycontextid, $form_status, $jsonformdata) {
        ...
        if ($validateddata) {
            ...
            if ($formheaders[$form_status] == 'location_date' && !empty($parentid)) {
                ...
                $categories = $DB->get_records('local_custom_fields_mapped', array('moduletype' => 'classroom', 'moduleid' => $data['id']));
                if ($categories) {
            ...
            if (method_exists(new classroom, $formheaders[$form_status])) {
                if ($formheaders[$form_status] == 'target_audience') {
    ...
    public static function delete_classroom_instance($action, $id, $confirm, $classroomname) {
        ...
        try {
            ...
            $DB->delete_records('local_custom_fields_mapped', array('moduletype' => 'classroom', 'moduleid' => $id));
        ...
        try {
            if ($confirm) {
                ...
                foreach ($classroom_users as $classroom_user) {
                    ...
                    $attendedsessions = $DB->count_records(
    ...
    public static function classroom_session_instance($id, $categorycontextid, $form_status, $jsonformdata) {
        ...
        $mform = new \local_classroom\form\session_form(
            ...
            array(
                'id' => $data['id'],
                'cid' => $data['classroomid'], 'form_status' => $form_status
            ),
            'post',
            '',
            null,
    ...
    public static function delete_classroomcourse_instance($action, $id, $classroomid, $confirm, $name) {
        ...
        try {
            if ($confirm) {
                ...
                $classroomtrainers = $DB->get_records_menu(
                ...
                if (!empty($classroomtrainers)) {
                    foreach ($classroomtrainers as $classroomtrainer) {
                        $unenrolclassroomtrainer = (new classroom)->manage_classroom_course_enrolments(
                            $course,
                            $classroomtrainer,
                            'editingteacher',
                            'unenrol',
                ...
                $classroomusers = $DB->get_records_menu(
                    'local_classroom_users',
                    array('classroomid' => $classroomid),
                    'userid',
                    'id, userid'
                );
                if (!empty($classroomusers)) {
                    foreach ($classroomusers as $classroomuser) {
                        $unenrolclassroomuser = (new classroom)->manage_classroom_course_enrolments(
                            $course,
                            $classroomuser,
                            'employee',
                            'unenrol',
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomviewwaitinglistusers(
        $categorycontextid,
        $options,
        $dataoptions,
        $offset = 0,
     ...
     * @param  [type]                limit [limit]
     */
    public static function classroomfeedbacks(
        $userid,
        $classroomid,
        $search = '',
        $page = 0,
    ...
    public static function get_classroom_info($id) {
        $params = self::validate_parameters(
            self::get_classroom_info_parameters(),
            ['id' => $id]
        );
