...
function local_classroom_output_fragment_classroom_form($args) {
    ...
    if ($parentid) {
        ...
        foreach ($parentid as $categoryid) {
            $mapedcategory = $DB->get_record('local_custom_fields_mapped', array('moduletype' => 'classroom', 'moduleid' => $data->id, 'parentid' => $categoryid->id));

            $customdata['customfield_' . $categoryid->id] = $mapedcategory ? $mapedcategory : 0;
        }
    ...
    $classroomdata = clone ((object)$data);
...
/**
 * costcenterwise_classroom_datacount to restrict deletion of parentelement
 *
