...
class renderer extends plugin_renderer_base {
    ...
    public function get_program_tabs($stable, $selectedprogram = null, $selectedstatus = null, $view_type = 'card') {
        ...
        $programtabslist = [
            ...
            'plugintype' => 'local',
            'plugin_name' => 'program',
            'is_siteadmin' => ((has_capability(
                'local/program:manageprogram',
                $categorycontext
            )) || is_siteadmin()) ? true : false,
            'creataprogram' => ((has_capability(
                'local/program:manageprogram',
                $categorycontext
            ) && has_capability(
    ...
    public function viewprogramlevels($programid, $levelid) {
        ...
        if (!empty($programlevels)) {
            ...
            foreach ($programlevels as $k => $programlevel) {
                ...
                if (
                    $userview && !is_siteadmin() && !has_capability('local/program:createprogram', $categorycontext) && array_search($programlevel->id, $mycompletedlevels) === false
        ...
        $return = $this->render_from_template(
            'local_program/levelstab_content',
            $programlevelscontext
        );
        return $return;
    }
    ...
    public function viewprogramcourses($programid, $levelid) {
        ...
        foreach ($programs as $program) {
            ...
            foreach ($programlevelcourses as $i => $bclevelcourse) {
                ...
                $bclevelcourse->course = html_writer::link(
                    $courseurl,
                    $courselink,
                ...
                if ($userview) {
                    $completion = $DB->get_field_sql('SELECT timecompleted FROM {course_completions} WHERE course=:courseid AND userid=:userid', array('courseid' => $bclevelcourse->id, 'userid' => $USER->id));
                    if ($completion) {
                        $bclevelcourse->coursecompleted = true;
                    }
        ...
        $return = $this->render_from_template(
            'local_program/levelcoursescontent',
            $programcoursescontext
        );
        return $return;
    }
    ...
    public function viewprogram($programid) {
        ...
        $componentname = 'local_program';
        $display_ratings = $output->course_rating_block_custom($programid, $componentname);
        $display_ratingsdefault = $output->course_rating_block($programid, $componentname);
    ...
    public function viewprogramusers($stable) {
        ...
        if (has_capability('local/program:manageusers', $categorycontext) && has_capability('local/program:manageprogram', $categorycontext)) {
            ...
            // <li>
            //        <div class="createicon course_extended_menu_itemlink"><a href="' . $CFG->wwwroot . '/local/program/users.php?download=1&amp;format=xls&amp;type=programwise&amp;bcid='.$programid.'&amp;search='.$stable->search.'" target ="_blank"><i class="icon fa fa-download" aria-hidden="true" title="'.get_string('programdownloadreport', 'local_program').'"></i></a>
            //        </div>
            // </li>
            // <li>
        ...
        if ($stable->thead) {
            ...
            foreach ($programusers['programusers'] as $sdata) {
                ...
                if (!empty($sdata->open_supervisorid)) {
                    $supervisor = $DB->get_record(
                        'user',
                        array('id' => $sdata->open_supervisorid),
    ...
    public function viewprogramlastchildpopup($programid) {
        ...
        $allocatedseats = $DB->count_records(
            'local_program_users',
            array('programid' => $programid)
        );
