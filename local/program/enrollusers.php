...
if ($programid) {
    ...
    if ($add && confirm_sesskey()) {
        if ($submit_value == "Add_All_Users") {
            ...
            $userstoassign = array_flip((new program)->select_to_and_from_users(
                'add',
                $programid,
                $options,
                false,
    ...
    if ($remove && confirm_sesskey()) {
        if ($submit_value == "Remove_All_Users") {
            $options = (array)json_decode($_REQUEST["options"], false);
            $userstounassign = array_flip((new program)->select_to_and_from_users(
                'remove',
                $programid,
                $options,
                false,
    ...
    $select_from_users_total = (new program)->select_to_and_from_users(
        'remove',
        $programid,
        $options,
        true,
