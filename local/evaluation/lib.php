...
function evaluation_add_instance($evaluation) {
    ...
    if ($draftitemid = $evaluation->page_after_submit_editor['itemid']) {
        $evaluation->page_after_submit = file_save_draft_area_files(
            $draftitemid,
            $context->id,
            'local_evaluation',
            'page_after_submit',
 ...
 */
function evaluation_get_incomplete_users(
    $evaluation,
    $group = false,
    $sort = '',
    $startpage = false,
...
function evaluation_get_depend_candidates_for_item($evaluation, $item) {
    ...
    $evaluationitems = $DB->get_records_select_menu(
        'local_evaluation_item',
        $where,
        $params,
        'position',
        'id, label'
...
function evaluation_create_item($data) {
    debugging(
        'Function evaluation_create_item() is deprecated because it was not used.',
        DEBUG_DEVELOPER
    );
    global $DB;
...
function evaluation_save_tmp_values($evaluationcompletedtmp, $evaluationcompleted) {
    ...
    foreach ($values as $value) {
        ...
        if ($item->dependitem > 0 && isset($allitems[$item->dependitem])) {
            $check = evaluation_compare_item_value(
                $tmpcplid,
...
function evaluation_delete_completedtmp($tmpcplid) {
    ...
    debugging(
        'Function evaluation_delete_completedtmp() is deprecated because it is no longer used',
        DEBUG_DEVELOPER
    );
 ...
 */
function evaluation_get_group_values(
    $item,
    $groupid = false,
    $courseid = false,
    $ignore_empty = false
...
function evaluation_get_current_completed(
    ...
    debugging(
        'Function evaluation_get_current_completed() is deprecated. Please use either ' .
...
function evaluation_print_numeric_option_list($startval, $endval, $selectval = '', $interval = 1) {
    debugging(
        'Function evaluation_print_numeric_option_list() is deprecated because it was not used.',
        DEBUG_DEVELOPER
    );
...
function get_listof_evalautions($stable, $filtervalues) {
    ...
    foreach ($records as $record) {
        ...
        $evalimg = $OUTPUT->image_url('/course_images/image3', 'local_costcenter');
        $line["evalimage"]  = $evalimg->out();
...
function evaluation_filter($mform, $query = '', $searchanywhere = false, $page = 0, $perpage = 25) {
    ...
    if (!empty($query)) {
        $evaluation_sql .= " AND name LIKE :ename ";
        $params['ename'] = '%' . trim($query) . '%';
    }
    ...
    if (!empty($query) || empty($mform)) {
        $evaluationlist = $DB->get_records_sql($evaluation_sql,  $params, $page, $perpage);
        return $evaluationlist;
    }
    if ((isset($data->departments) && !empty($data->departments))) {
        $evaluationlist = $DB->get_records_sql_menu($evaluation_sql,  $params, $page, $perpage);
    }
