...
class local_evaluation_responses_table extends table_sql {
    ...
    public function __construct(local_evaluation_structure $evaluationstructure, $group = 0) {
        ...
        $this->define_baseurl(new moodle_url(
            '/local/evaluation/show_entries.php',
        ...
        $this->is_downloading(
            ...
            $name,
            get_string('responses', 'local_evaluation')
        );
    ...
    protected function init($group = 0) {
        ...
        $extrafields = \core_user\fields::for_identity($this->get_context())->get_required_fields();
        $userfields = \core_user\fields::get_identity_fields($this->get_context()); //user_picture::fields('u', $extrafields, $this->useridfield);
        if (!empty($userfields)) {
            $ufields = implode(',',  $userfields);
        }
        $fields = 'c.id, c.timemodified as completed_timemodified, c.evaluatedby as customcompleted_evaluateduser, u.firstname,u.lastname ' . $ufields;
    ...
    public function col_deleteentry($row) {
        ...
        return $OUTPUT->action_icon(
            $deleteentryurl,
            new pix_icon('t/delete', get_string('delete_entry', 'local_evaluation')),
            $deleteaction
        );
    }
    ...
    public function col_completed_timemodified($student) {
        if ($this->is_downloading()) {
            ...
            return html_writer::link(
                $this->get_link_single_entry($student),
    ...
    public function display() {
        ...
        $this->out(
            $this->showall ? $grandtotal : EVALUATION_DEFAULT_PAGE_COUNT,
        ...
        if ($this->totalrows > EVALUATION_DEFAULT_PAGE_COUNT) {
            if (!$this->use_pages) {
                echo html_writer::div(html_writer::link(
                    ...
                    get_string('showperpage', '', EVALUATION_DEFAULT_PAGE_COUNT)
                ), 'showall');
            } else {
                echo html_writer::div(html_writer::link(
