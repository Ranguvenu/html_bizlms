...
if ($course) {
    ...
    $options = array('context' => $context->id, 'courseid' => $course_id, 'organization' => $organization, 'department' => $department, 'subdepartment' => $subdepartment, 'department4level' => $department4level, 'department5level' => $department5level, 'email' => $email, 'idnumber' => $idnumber, 'uname' => $uname, 'groups' => $groups, 'designation' => $designation, 'hrmsrole' => $hrmsrole, 'location' => $location, 'states' => $states, 'district' => $district, 'subdistrict' => $subdistrict, 'village' => $village);
    $dataobj = $course_id;
    $fromuserid = $USER->id;
    ...
    if ($remove && confirm_sesskey()) {
        ...
        if (!empty($userstounassign)) {
            ...
            foreach ($userstounassign as $key => $removeuser) {
                ...
                $enrol_self = enrol_get_plugin('auto');
                if ($data_self->enrol == 'auto') {
                    $self = $enrol_self->unenrol_user($data_self, $removeuser);
                }
                $notification = new \local_courses\notification();
                $course = $DB->get_record('course', array('id' => $dataobj));
  ...
  <label>' . get_string('enrolled_users', 'local_courses', $select_from_users_total) . '</label>' . $select_all_not_enrolled_users;
    ...
    $content .= '</select></div>';
    $content .= '<div class="box3 col-md-2 col-12 pull-left actions">';

    $content .= '<button type="submit" class="custom_btn btn remove btn-default" disabled="disabled" title="Remove Selected Users" name="submit_value" value="Remove Selected Users" id="user_unassign_all"/>
  ' . get_string('remove_selected_users', 'local_courses') . '
  </button></form>';

    $content .= '<form  method="post" name="form_name" id="user_un_assign" class="form_class" >';
    $content .= '<button type="submit" class="custom_btn btn move btn-default" disabled="disabled" title="Add Selected Users" name="submit_value" value="Add Selected Users" id="user_assign_all" />
  ' . get_string('add_selected_users', 'local_courses') . '
  </button>';
    if (isset($roles[$roleid])) {

        $content .= '<label for="roleid"> ' . get_string('assignrole', 'enrol_manual') . '</label>';
        $content .=    html_writer::select($roles, 'roleid', $roleid, false, array('class' => 'enrolselect'));
    }
