...
class local_courses_external extends external_api {
    ...
    public static function submit_create_course_form($contextid, $form_status, $id, $jsonformdata) {
        ...
        $params = self::validate_parameters(
            self::submit_create_course_form_parameters(),
        ...
        $customfields = $DB->get_records('local_custom_fields_mapped', array('moduletype' => 'course', 'moduleid' => $data['id']));
        $data['childfieldid'] = [];
        foreach ($customfields as $parentfield) {
            $data['childfieldid'][$parentfield->parentid] = $parentfield->id;
        }

        if (!empty($data['performancecatid'])) {
            ...
            $data['open_categoryid'] = $data['performancecatid'];
            ...
            if ($validateddata->id <= 0) {
                ...
                if ($form_status == 0) {
                    ...
                    if ($form_status == 2) {
                        ...
                        $data->open_group = ($data->open_group == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$data->open_group);
                        $data->open_designation = ($data->open_designation == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$data->open_designation);
                        $data->open_hrmsrole = ($data->open_hrmsrole == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$data->open_hrmsrole);
                        $data->open_project = ($data->open_project == '_qf__force_multiselect_submission') ? NULL : implode(',', (array)$data->open_project);
                    } else {
                    ...
                    if ($form_status == 1) {
                        ...
                        $data->childfieldid = $validateddata->childfieldid;
    ...
    public static function delete_course($action, $id, $confirm, $name) {
        ...
        try {
            if ($confirm) {
                ...
                $DB->delete_records('local_custom_fields_mapped', array('moduletype' => 'course', 'moduleid' => $id));
                $return = true;
            } else {
                $return = false;
            }
  ...
  */
    ...
    public static function get_course_info($id) {
        global $DB;
        $params = self::validate_parameters(
            self::get_course_info_parameters(),
            ['id' => $id]
        );
