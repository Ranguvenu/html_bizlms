...
class custom_course_form extends moodleform {
    ...
    function definition() {
        ...
        $childfieldid = $this->_customdata['childfieldid'];
        ...
        $mform->addElement('hidden', 'childfieldid', $childfieldid);
        $mform->setType('childfieldid', PARAM_RAW);
        ...
        if ($formstatus == 0) {
            ...
            if ($plugin_exists) {
                ...
                // Add the 'Select Category' option at the top.
                $performance_types[0] = array(
                    0 => get_string('select_ptype', 'local_courses')
                );
                ...
                if ($ptype_categories) {
                    foreach ($ptype_categories as $ptype_category) {
                        // Initialize the child categories array for each parent.
                ...
                // Add the grouped select dropdown to the form.
                $mform->addElement('selectgroups', 'performancecatid', get_string('performance_type', 'local_courses'), $performance_types);
            ...
            if (isset($course->format)) {
                ...
                if (!in_array($course->format, $courseformats)) {
                    ...
                    $formcourseformats[$course->format] = get_string(
                        'withdisablednote',
                        'moodle',
            ...
            get_custom_fields($costcenterid, $mform, $moduletype = 'course');
            ...
            $mform->addElement(
                'date_time_selector',
                'startdate',
                get_string('startdate', 'local_courses'),
                array()
            );
            $mform->addHelpButton('startdate', 'startdate');

            $default_timestamp = time() + 86400;

            $mform->addElement('date_time_selector', 'enddate', get_string('enddate', 'local_courses'), array(
                'defaulttime' => $default_timestamp,
                'optional' => true,
            ));
    ...
    function validation($data, $files) {
        ...
        if ($data['form_status'] == 0) {
            ...
            if (isset($data['performancecatid']) && $data['form_status'] == 0) {
                if ($data['performancecatid'] == 0) {
        ...
        if ($data['form_status'] == 1) {
            if (isset($data['startdate']) && !empty($data['enddate'])) {
