...
function local_groups_get_groups($contextid, $page = 0, $perpage = 10, $search = '') {
    ...
    if (isset($search->search_query) && trim($search->search_query) != '') {
        $sql .= " AND (c.name LIKE :search1)";
        $params['search1'] = '%' . trim($search->search_query) . '%';
    }
    $allgroups = '';
...
function groups_filter($mform, $query = '', $searchanywhere = false, $page = 0, $perpage = 25) {
    ...
    if (!empty($query)) {
        if ($searchanywhere) {
            $groupslist_sql .= " AND c.name LIKE :gquery ";
        } else {
            $groupslist_sql .= " AND c.name LIKE :gquery ";
        }
        $groupsparam['gquery'] = '%' . trim($query) . '%';
    }
