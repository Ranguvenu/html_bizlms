...
function get_listof_forum($stable, $filterdata, $options) {
    ...
    if (is_siteadmin() || has_capability('local/forum:manage', $maincheckcontext)) {
        ...
        $formsql = " JOIN {enrol} AS e ON c.id = e.courseid AND e.enrol NOT IN ('classroom', 'program', 'learningplan')
                    JOIN {user_enrolments} ue ON e.id = ue.enrolid AND  ue.userid = {$USER->id} ";
    }
    $open_path = (new \local_costcenter\lib\accesslib())::get_costcenter_path_field_concatsql($columnname = 'c.open_path');
    $wheresql = " WHERE c.id <> 1 AND c.visible = 1 AND c.open_module ='forum' AND c.open_coursetype = 1 ";
    if (!is_siteadmin()) {
        $wheresql .= $open_path;
    }
    ...
    $totalcourses = $DB->count_records_sql($countsql . $formsql . $wheresql, $params);

    $ordersql = " ORDER BY c.id DESC";
    $courses = $DB->get_records_sql($selectsql . $formsql . $wheresql . $ordersql, $params, $stable->start, $stable->length);
    ...
    if (!empty($courses)) {
        ...
        foreach ($courses as $key => $course) {
            ...
            // $categoryname = $DB->get_field('local_custom_fields', 'fullname', array('id' => $course->category));
            $categoryname = $DB->get_field('local_custom_category', 'fullname', array('id' => $course->category));
            $departmentcount = 1;
...
function local_forum_leftmenunode() {
    ...
    } else if (!is_siteadmin() && has_capability('local/forum:view', $categorycontext)) {
    ...
    return array('10' => $coursecatnodes);
}
...
function forum_filters_form($filterparams, $formdata = []) {
    ...
    if (is_siteadmin()) {
        $mform = new filters_form(null, array(
            'filterlist' => array('hierarchy_fields',/*'geographyfields',*/ 'forum', 'status', 'categories', 'customfields'),
            'courseid' => 1,
            'enrolid' => 0,
            'plugins' => array('forum', 'costcenter'),
            'filterparams' => $filterparams
        ) + $formdata);
    } else {
        $filters = array('hierarchy_fields',/* 'geographyfields',*/ 'forum', 'status', 'categories', 'customfields');
