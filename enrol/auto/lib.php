...
class enrol_auto_plugin extends enrol_plugin {
    ...
    public function try_autoenrol(stdClass $instance) {
        global $USER, $DB;
        if ($instance->customint3 == ENROL_AUTO_DISABLED) {
            return false;
        }
        ...
        $user_sql = "SELECT id,open_departmentid,open_hrmsrole,open_ouname,
                        open_designation,city FROM {user} where id=$USER->id";
        ...
        if ($DB->record_exists_sql($sql) && $DB->record_exists_sql($course_sql)) {
