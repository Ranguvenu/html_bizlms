...
/**
 * Test the epsilon_classicnavbar file
 *
 * @package    theme_epsilon_classic
 * @covers     \theme_epsilon_classic\epsilon_classicnavbar
 * @copyright  2021 Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class epsilon_classicnavbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected) {
        ...
        $epsilon_classicnavbar = $this->getMockBuilder(epsilon_classicnavbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(epsilon_classicnavbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setAccessible(true);
        $rcp->setValue($epsilon_classicnavbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->setAccessible(true);
        $rcm->invoke($epsilon_classicnavbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($epsilon_classicnavbar);
        $actual = [];
    ...
    public function test_remove_duplicate_items(array $navbarnodes, array $expected) {
        ...
        $rcm->setAccessible(true);
        $rcm->invoke($epsilon_classicnavbar);
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->setAccessible(true);
        $rcm->invoke($epsilon_classicnavbar, $navigationmenu);
