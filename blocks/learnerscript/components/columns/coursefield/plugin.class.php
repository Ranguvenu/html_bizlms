    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        switch ($data->column) {
            ...
            case 'coursecode':
                ...
                break;
            case 'coursecategory':
                $parentname = $DB->get_field_sql("SELECT slcc.fullname FROM {local_custom_category} lcc
                JOIN {local_custom_category} slcc ON lcc.parentid=slcc.id AND slcc.parentid = 0 AND lcc.id = :catid", ['catid' => $row->open_categoryid]);
                $coursecategory = $parentname . ' / ' . $this->coursecategory[$row->open_categoryid];
