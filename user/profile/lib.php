...
class profile_field_base {
    ...
    public function edit_field_set_required($mform) {
        ...
        // }
        ////Add display the required fields in first tab of custom user form <Revathi>
         if ($this->is_required()){ //newly added for user custom fileds as target audience
            $mform->addRule($this->inputname, get_string('required'), 'required', null, 'client');
        }
        //end<Revathi>
    }
    ...
    public function is_editable() {
        ...
        //Add <revathi>
         ...
         if (has_capability('moodle/user:update', $systemcontext) || has_capability('moodle/user:update', $categorycontext) ) {
            return true;
        }
        //End <Revathi>
 ...
 * @param int $userid
 * @return profile_field_base[]
  * @param int $form_status to display the required fields in first tab of custom user form
 */
function profile_get_user_fields_with_data(int $userid,int $form_status = 0): array {
    global $DB;
    ...
    $profileinfocatid = \local_costcenter\lib\accesslib::get_user_profile_category($userid);
    $sql .= " WHERE uif.categoryid = :infocatid ";
    if($form_status == 1){
        $sql .= " AND uif.required = 0 ";
    }
    //end<revathi>
    $sql .= 'ORDER BY uic.sortorder ASC, uif.sortorder ASC ';
    $fields = $DB->get_records_sql($sql, ['userid' => $userid, 'infocatid' => $profileinfocatid]);//, 'infocatid' => $profileinfocatid]<Revathi>
    $data = [];
 ...
 * @param int $form_status to display the required fields in first tab of custom user form
 */
function profile_get_user_fields_with_data_by_category(int $userid, int $form_status = 1): array  {
    $fields = profile_get_user_fields_with_data($userid,$form_status);//$form_statu<Revathi>
    $data = [];
    foreach ($fields as $field) {
...
function profile_load_data(stdClass $user): void {
    ...
    // }
    //Add display the required fields in first tab of custom user form <Revathi>
     $costcenterid = (!empty($user->open_costcenterid)) ? $user->open_costcenterid : 0;
    $fields = profile_get_user_fields_with_data($user->id,$costcenterid);
    foreach ($fields as $formfield) {
        $formfield->edit_load_user_data($user);
    }
    //End<Revathi>
...
function profile_definition(MoodleQuickForm $mform, int $userid = 0, int $form_status = 1,int $open_costcenterid = 0): void{
    ...
    //Add display the required fields in first tab of custom user form <Revathi>
     /*  if($userid != 0){
            $categories = profile_get_user_fields_with_data_by_category($userid,$open_costcenterid);
        }else{
            $fields = get_usercustomfield_category_bycostcenter($open_costcenterid);
            $categories = [];
            foreach ($fields as $field) {
                $categories[$field->field->categoryid][] = $field;
            }
        } */
       if($form_status == 0 && $open_costcenterid != 0){
       
            $fields = get_usercustomfield_category_bycostcenter($userid,$form_status,$open_costcenterid);
            $categories = [];
            foreach ($fields as $field) {
                $categories[$field->field->categoryid][] = $field;
            }
       }else{
            $categories = profile_get_user_fields_with_data_by_category($userid,$form_status);
       }
        ...
        foreach ($categories as $categoryid => $fields) {
            ...
            foreach ($fields as $formfield) { 
                ...
                if($formfield->is_required()){
                    $requiredfieldstodisplay[] = $formfield;
                }
            }
            ...
            if($form_status == 0){
                foreach ($requiredfieldstodisplay as $reqformfield) {            
                    $reqformfield->edit_field($mform);
                }
            }else{
                foreach ($fieldstodisplay as $formfield) {            
                    $formfield->edit_field($mform);
...
function profile_save_data(stdClass $usernew): void {
    ...
    foreach ($fields as $formfield) {
       //Add display the required fields in first tab of custom user form <Revathi>
         if($formfield instanceof \profile_field_checkbox){
            $usernew->{$formfield->inputname} = $usernew->{$formfield->inputname} == 1 ? 'Yes' : 'No';
        }
        //End <revathi>
